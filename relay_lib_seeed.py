...
# =========================================================
# Modified by Raoul Mackle raoul.mackle@kultivator.co.nz to
# add logging.
...
logger = logging.getLogger('seed_relay')
...
def relay_on(relay_num):
    ...
    if isinstance(relay_num, int):
        ...
        if 0 < relay_num <= NUM_RELAY_PORTS:
            logger.info('Turning relay' + str(relay_num) + 'ON')
            DEVICE_REG_DATA &= ~(0x1 << (relay_num - 1))
            bus.write_byte_data(DEVICE_ADDRESS, DEVICE_REG_MODE1, DEVICE_REG_DATA)
        else:
            logger.error('Invalid relay #:' + str(relay_num))
    else:
        logger.error('Relay number must be an Integer value')
...
def relay_off(relay_num):
    ...
    if isinstance(relay_num, int):
        # do we have a valid relay number?
        if 0 < relay_num <= NUM_RELAY_PORTS:
            logger.info('Turning relay' + str(relay_num) + 'OFF')
            DEVICE_REG_DATA |= (0x1 << (relay_num - 1))
            bus.write_byte_data(DEVICE_ADDRESS, DEVICE_REG_MODE1, DEVICE_REG_DATA)
        else:
            logger.error('Invalid relay #:' + str(relay_num))
...
def relay_all_on():
    ...
    logger.info('Turning all relays ON')
...
def relay_all_off():
    ...
    logger.info('Turning all relays OFF')
    DEVICE_REG_DATA |= (0xf << 0)
    bus.write_byte_data(DEVICE_ADDRESS, DEVICE_REG_MODE1, DEVICE_REG_DATA)


def relay_toggle_port(relay_num):
    logger.info('Toggling relay:' + str(relay_num))
...
def relay_get_port_status(relay_num):
    ...
    if res > 0:
        ...
        logger.error("Specified relay port is invalid")
        return False
